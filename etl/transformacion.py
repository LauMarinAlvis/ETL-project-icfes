...
# Código centinela para estudiantes inscritos de forma individual (sin colegio)
COD_ICFES_SIN_COLEGIO = -1

# Centinela para NSE faltante
NSE_SIN_DATO = -1
...
def limpiar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reparación con ftfy, limpieza de espacios y reemplazo de nulos en texto
    columnas_texto = COLUMNAS_CATEGORICAS_DIM + ["cole_nombre_establecimiento"]
    for col in columnas_texto:
        if col in df.columns:
            df[col] = df[col].astype("string").apply(lambda x: ftfy.fix_text(x) if pd.notna(x) else x)
    ...
    log.info("Filas descartadas por llaves nulas (estudiante/periodo): %d", filas_antes - len(df))

    # Priorización del registro institucional sobre la inscripción individual duplicada
    ...
    log.info("Filas descartadas por doble inscripción (individual + institución): %d", filas_antes - len(df))

    duplicados_restantes = int(df["estu_consecutivo"].duplicated().sum())
    if duplicados_restantes:
        log.warning("Quedan %d estu_consecutivo duplicados sin explicar. Revisar manualmente.", duplicados_restantes)
...
def construir_fact(df, dim_colegio, dim_ubicacion, dim_socioeco, dim_tiempo) -> pd.DataFrame:
    ...
    if sin_fk:
        log.warning(
            "%d filas quedaron sin alguna llave foránea (no calzaron con una dimensión) y serán descartadas.",
            sin_fk
        )
